...
def set_window(udp):
    ...
    label1.place(x=15,y = 110)

    e1 = Entry(window,width = 5, font = ('Helvetica',12),fg = inlan_color)
    e1.focus_set()
    e1.insert(0,'0,0') 
    e1.place(x = 165, y = 112)
    ...
    e2 = Entry(window,width = 5, font = ('Helvetica',12),fg = inlan_color)
    e2.insert(0,'10,0') 
    e2.place(x = 165, y = 152)
    ...
    e3 = Entry(window,width = 5, font = ('Helvetica',12),fg = inlan_color)
    e3.insert(0,'0,15') 
    e3.place(x = 165, y = 192)
    ...
    label4.place(x = 15, y = 230)
    e4 = Entry(window, width = 5, font = ('Helvetica',12), fg = inlan_color)
    e4.insert(0,'10,15') 
    e4.place(x = 165, y = 232)
    entry_list.append(e4)

    b1 = Button(window, text = 'Set locations' , command = lambda: start_display(udp,entry_list, window, canvas), width = 20,bg = inlan_color, fg = 'white', font = ('Helvetica',12))
    b1.place(x = 20,y = 270)
    ...
    return entry_list, b1, window, canvas


def setup_environment(udp,window,canvas,amps_avg,rx_info, csv_path = 'location_data.csv'):
    rx_list = []
    # window = Tk()
    inlan_color = '#0A0D35'
    angle_str = ""
    ...
    score_label  = Label(window, text = f'{angle_str} Tag Location (x,y) = ({np.round(x_est_avg,1)}, {np.round(y_est_avg,1)})', font=("Helvetica", 12), fg = inlan_color)
    score_label.place(x = 5, y = 420)

    return score_label, rx_list, tag
...
def update_step(udp, amps_avg, rx_list, tag, score_label):
    ...
    udp.current_est_size += 1
    x_est_avg, y_est_avg = np.sum(udp.est_buffer,axis=1) / min(udp.est_window,udp.current_est_size)
    score_label.config(text= f'Tag Location (x,y) = ({np.round(x_est_avg,2)}, {np.round(y_est_avg,2)})', font=("Helvetica", 12))
...
def send_prompts(udp,rx_info):
    ...
    for i in range(num_rx):
        ...
        x_mag_db = -float(message[2*i+1]) #add 1 to not use anything before the first '-'
    ...
    for i, entry in enumerate(entry_list):
        ...
        if len(text) != 0:
            ...
            info = {
                ...
                'color': color_list[i],
                'noise floors': (-300,-300),
                'port': str(i)
            }
            rx_info.append(info)
    return rx_info

def start_display(udp,entry_list, window, canvas):
    ...
    amps_avg = send_prompts(udp = udp, rx_info = rx_info)
    score_label, rx_list, tag = setup_environment(udp,window,canvas,amps_avg,rx_info, csv_path = 'location_data.csv')
    #Display loop
...
if __name__ == '__main__':
    ...
    #Set the window
    # button.configure(command = lambda: start_display(entry_list, window, canvas))
    #Receiver specs
    # rx_info = []
    # info_rx0 = {
    #     'location': (0,0),
    ...
    NUM_RX = 2
    udp = UDPConn(s, (host, port),num_rx = NUM_RX,use_lookup= False, n = 1)
    th = threading.Thread(target=read_thread,args=(s, "name",udp))
    th.start()

    entry_list, button, window, canvas = set_window(udp)
